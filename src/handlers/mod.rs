...
///
/// # Для студентов: добавление нового эндпоинта на готовый стек
///
/// Этот обработчик — пример того, как расширять учебный демо-проект без
/// модификации существующих маршрутов:
///
/// 1. Добавили модели `SummarizeRequest` / `SummarizeResponse` в `models`.
/// 2. Написали handler с макросом `#[post("/summarize", format = "json", …)]`.
/// 3. Подключили его в `main.rs` через `routes![…, summarize, …]`.
/// 4. Договорились о протоколе с `AiService`: handler формирует промпт по
///    шаблону, MockAiService узнаёт этот шаблон и отвечает структурированно.
///
/// # Договор с AiService (как handler «понимает» ответ)
///
/// Handler формирует промпт, начинающийся со слов
/// «Сделай краткий конспект …». В `MockAiService` есть зеркальная ветка,
...
/// ```
///
/// `parse_summary_payload` разбирает её обратно в `SummarizeResponse`.
/// Если ответ AI не содержит маркеров (например, `GigaChatService`
/// сгенерировал свой свободный текст), handler возвращает этот текст
/// как `summary`, а `key_terms` оставляет пустым. Это безопасный фолбэк.
///
/// # Эндпоинт
///
/// `POST /summarize`
