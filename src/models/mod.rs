...
// ============================================================================
// МОДЕЛИ ДЛЯ /summarize («Умный конспектор»)
// ============================================================================

/// Запрос на конспектирование текста.
///
/// Принимается обработчиком `POST /summarize`. Поле `text` обязательно,
/// `max_terms` — опциональное ограничение на размер списка ключевых терминов
/// в ответе (если клиент не указал — handler берёт значение по умолчанию).
#[derive(Debug, Deserialize)]
#[serde(crate = "rocket::serde")]
pub struct SummarizeRequest {
    ...
    /// Если не передано — handler использует значение по умолчанию (5).
    #[serde(default)]
...
///
/// Поле `source` совпадает по смыслу с одноимённым полем `AskResponse` —
/// показывает, откуда пришёл результат (`gigachat` или `mock ai service`).
#[derive(Debug, Serialize)]
#[serde(crate = "rocket::serde")]
pub struct SummarizeResponse {
    ...
    /// Список ключевых терминов (длина ≤ запрошенному `max_terms`).
